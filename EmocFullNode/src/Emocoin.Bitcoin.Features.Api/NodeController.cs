...
using Emocoin.Bitcoin.Utilities.JsonErrors;
using Emocoin.Bitcoin.Utilities.ModelStateErrors;
...
namespace Emocoin.Bitcoin.Features.Api
{
    ...
    public class NodeController : Controller
    {
        ...
        /// <summary>
        /// Gets the block emoji type and emoji display identified by block height.
        /// </summary>
        /// <param name="hash">The height of the block to retrieve.</param>
        /// <param name="isJsonFormat">A flag that specifies whether to return the block header in the JSON format. Defaults to true. A value of false is currently not supported.</param>
        /// <returns>Json formatted <see cref="BlockHeaderModel"/>. <c>null</c> if block not found. Returns <see cref="Microsoft.AspNetCore.Mvc.IActionResult"/> formatted error if fails.</returns>
        /// <exception cref="NotImplementedException">Thrown if isJsonFormat = false</exception>"
        /// <exception cref="ArgumentException">Thrown if hash is empty.</exception>
        /// <exception cref="ArgumentNullException">Thrown if logger is not provided.</exception>
        /// <remarks>Binary serialization is not supported with this method.</remarks>
        [Route("getblockrewardemoji")]
        [HttpGet]
        public IActionResult GetBlockRewardEmoji([FromQuery] int height=0)
        {
            try
            {
                this.logger.LogDebug("GetBlockRewardEmoji {0}", height);

                string emoji = "";                
                if(height <=0)
                {
                    height = this.chainIndexer.Height + 1;
                }
                Emoji emo = Emoji.GetSlotsByBlockHeight(height-1);
                emoji = emo.ToString();

                BlockRewardEmojiModel model = new BlockRewardEmojiModel(height, emoji, emo.ToStringUnicode());
                return this.Json(model);
            }
            catch (Exception e)
            {
        ...
        [Route("getemoji")]
        [HttpGet]
        public IActionResult GetEmoji([FromQuery] int height=0)
        {
            try
            {
                this.logger.LogDebug("GetEmoji {0}", height);
                ...
                BlockRewardEmojiModel model = new BlockRewardEmojiModel(height, emoji, emo.ToStringUnicode());
                return this.Json(model.display);
            }
            catch (Exception e)
            {
                this.logger.LogError("Exception occurred: {0}", e.ToString());
