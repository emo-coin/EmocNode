...
namespace Emocoin.Bitcoin.Features.ColdStaking
{
    /// <summary>
    /// Feature for cold staking which eliminates the need to keep the coins in the hot wallet.
    /// </summary>
    /// <remarks>
    /// <para>In order to produce blocks on Emocoin network, a miner has to be online with running
    ...
    /// </summary>
    /// <exception cref="InvalidOperationException">Thrown if this is not a Emocoin network.</exception>
    public static class FullNodeBuilderColdStakingExtension
    {
        ...
        {
            // Ensure that this feature is only used on a Emocoin network.
            if (!fullNodeBuilder.Network.Consensus.IsProofOfStake)
                throw new InvalidOperationException("Cold staking can only be used on a Emocoin network.");
