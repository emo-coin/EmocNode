...
namespace Emocoin.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    public abstract class CoinViewRule : FullValidationConsensusRule
    {
        ...
        private Emoji determin_recept_emoji_slots(List<TxOut> coins, long totalToSend)
        {
            Dictionary<uint, double> emojiBals = new Dictionary<uint, double>();
            long sum = 0;

            //split based on the emoji
            foreach (TxOut coin in coins)
            {
                var genes = coin.Emoji.Genes();
                long used = Math.Min(totalToSend - sum, coin.Value);
            ...
            {              
                foreach(var code in emojiBals.Keys)
                {
            ...
            List<Tuple<uint, double>> balList = new List<Tuple<uint, double>>();
            foreach(var bal in emojiBals)
            {
                balList.Add(new Tuple<uint, double>(bal.Key, bal.Value));
            }
        ...
        public bool CheckEmojis(Transaction transaction, UnspentOutputSet view)
        {

            List<TxOut> inputs = new List<TxOut>();
            ...
            var expectedEmoji = determin_recept_emoji_slots(inputs, transaction.TotalOut);

            for (int ii = 0; ii < transaction.Outputs.Count; ii++)
            {
                TxOut txOut = transaction.Outputs[ii];
                if (txOut.IsEmpty || txOut.Value == Money.Zero ||
                    txOut.Emoji == null || txOut.Emoji.isEmpty())
                    continue;
