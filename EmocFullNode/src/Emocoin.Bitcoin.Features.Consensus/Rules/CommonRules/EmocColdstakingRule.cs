...
namespace Emocoin.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    /// </list></para></remarks>
    public class EmocColdstakingRule : UtxoStoreConsensusRule
    {
        /// <inheritdoc />
        ...
        public override Task RunAsync(RuleContext context)
        {
            ...
            for (int i = 2; i < coinstakeTransaction.Outputs.Count; i++)
            {
                if (scriptPubKey != coinstakeTransaction.Outputs[i].ScriptPubKey)
                {
                    ...
                    if (coinstakeTransaction.Outputs[i].ScriptPubKey == EmocCoinstakeRule.CirrusRewardScript && !cirrusScriptFlagSeen)
                    {
