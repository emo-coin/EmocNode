...
namespace Emocoin.Bitcoin.Features.MemoryPool.Rules
{
    /// <summary>
    /// Validates the transaction with the coin view.
    /// Checks if already in coin view, and missing and unavailable inputs.
    /// </summary>
    public class EmocCoinViewMempoolRule : CheckCoinViewMempoolRule
    {
        public EmocCoinViewMempoolRule(Network network,
            ITxMempool mempool,
        ...
        /// <remarks>Also see <see cref="EmocCoinviewRule"/></remarks>>
        public override void CheckTransaction(MempoolValidationContext context)
        {
            ...
            foreach (TxIn txin in context.Transaction.Inputs)
            {
                ...
                if (unspentOutput.Coins.TxOut.ScriptPubKey == EmocCoinstakeRule.CirrusRewardScript)
                {
