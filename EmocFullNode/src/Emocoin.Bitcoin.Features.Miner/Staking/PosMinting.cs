...
namespace Emocoin.Bitcoin.Features.Miner.Staking
{
    ...
    public class PosMinting : IPosMinting
    {
        ...
        /// <seealso cref="https://github.com/emocoinproject/EmocoinBitcoinFullNode/issues/1180"/>
        /// </summary>
        ...
        public async Task<bool> CreateCoinstakeAsync(List<UtxoStakeDescription> utxoStakeDescriptions, BlockTemplate blockTemplate, ChainedHeader chainTip, long searchInterval, long fees, CoinstakeContext coinstakeContext)
        {
            ...
            coinstakeContext.CoinstakeTx.Outputs.Add(new TxOut(Money.Zero, new Script(), emoji));
            ...
            long coinstakeOutputValue = coinstakeInput.TxOut.Value;
        ...
        //    coinstakeContext.CoinstakeTx.Outputs[lastOutputIndex].Emoji = emoji;
        //    //for (int i=0; i< coinstakeContext.CoinstakeTx.Outputs.Count; i++)
        //    //    coinstakeContext.CoinstakeTx.Outputs[i].Emoji = emoji;

        //    for (int i = 0; i < SplitFactor - 1; i++)
        //    {
        //        var split = new TxOut(splitValue, coinstakeContext.CoinstakeTx.Outputs[lastOutputIndex].ScriptPubKey, emoji);
        ...
        public virtual Transaction PrepareCoinStakeTransactions(int currentChainHeight, CoinstakeContext coinstakeContext, long coinstakeOutputValue, int utxosCount, long amountStaked, long reward)
        {
            ...
            long remainder = coinstakeOutputValue - (SplitFactor - 1) * splitValue;
            coinstakeContext.CoinstakeTx.Outputs[lastOutputIndex].Value = remainder;
            var emoji = coinstakeContext.CoinstakeTx.Outputs[lastOutputIndex].Emoji;
                ...
                for (uint n = 0; n < searchInterval; n++)
                {
                    ...
                    try
                    {
                        ...
                        if (context.Result.SetKernelFoundIndex(context.Index))
                        {
                            ...
                            context.CoinstakeContext.CoinstakeAddress = utxoStakeInfo.Address;                            
                            ...
                            // Support for otherwise unsupported script types.
                            {
                                context.CoinstakeContext.CoinstakeTx.Outputs.Add(new TxOut(Money.Zero,
                                    new Script(OpcodeType.OP_RETURN, Op.GetPushOp(utxoStakeInfo.Key.PubKey.Compress().ToBytes())), utxoStakeInfo.TxOut.Emoji));

                                scriptPubKeyOut = utxoStakeInfo.TxOut.ScriptPubKey;
                            }
                            
                            context.CoinstakeContext.CoinstakeTx.Outputs.Add(new TxOut(0, scriptPubKeyOut, utxoStakeInfo.TxOut.Emoji));
