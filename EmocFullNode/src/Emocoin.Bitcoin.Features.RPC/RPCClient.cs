...
using Emocoin.Bitcoin.Features.RPC.Exceptions;
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public async Task<PeerInfo[]> GetEmocoinPeersInfoAsync()
        {
            RPCResponse resp = await SendCommandAsync(RPCOperations.getpeerinfo).ConfigureAwait(false);
            var peers = resp.Result as JArray;
            var result = new PeerInfo[peers.Count];
        ...
        /// <remarks>This method is not compatible with emocoinX, which always returns a JSON object no matter what parameters are passed to the second argument.</remarks>
        ...
        public async Task<Block> GetBlockAsync(uint256 blockId)
        {
            ...
            // emocoind, however, only accepts a bool. And it unconditionally returns a JSON object, which is not useable for parsing back into a block.
