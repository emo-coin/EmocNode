...
using Emocoin.Bitcoin.Features.Wallet.Controllers;
using Emocoin.Bitcoin.Features.Wallet.Helpers;
using Emocoin.Bitcoin.Features.Wallet.Interfaces;
using Emocoin.Bitcoin.Features.Wallet.Models;
using Emocoin.Bitcoin.Utilities;

namespace Emocoin.Bitcoin.Features.Wallet.Services
{
    public class WalletService : IWalletService
    {
            ...
            CancellationToken cancellationToken)
        {
            return await Task.Run(() =>
            {
                ...
                return new WalletGeneralInfoModel
                {
                    ...
                    Maturity = (int)wallet.Network.Consensus.CoinbaseMaturity,
        ...
        public async Task<WalletHistoryModel> GetHistory(WalletHistoryRequest request, CancellationToken cancellationToken)
        {
            return await Task.Run(() =>
            {
                int maxCoinBaseHeight = this.chainIndexer.Tip.Height - (int)this.network.Consensus.CoinbaseMaturity;
                ...
                foreach (AccountHistory accountHistory in accountsHistory)
                {
                    ...
                    foreach (FlatHistory item in items.Where(t => !t.Address.IsChangeAddress() || (t.Address.IsChangeAddress() && t.Transaction.IsSpent())))
                    {
                            ...
                            transaction.SpendingDetails.IsCoinStake.Value)
                        {
                            ...
                            // This will only ever happen when there are multiple inputs in a CoinStake. EmocoinX does this in certain situations.
        ...
        public async Task<WalletBuildTransactionModel> BuildTransaction(BuildTransactionRequest request, CancellationToken cancellationToken = default(CancellationToken))
        {
            return await Task.Run(() =>
            {
                ...
                var emojis = request.Emojis.Select(emojiStr => new Emoji(emojiStr)).ToList();
                ...
                var context = new TransactionBuildContext(this.network)
                {
                    ...
                    Emojis = emojis //Storing emoji from request                
                };
                ...
                try
                {
                    string accountExtPubKey =
                        this.network.IsBitcoin()
                            ? request.ExtPubKey
                            : LegacyExtPubKeyConverter.ConvertIfInLegacyEmocoinFormat(request.ExtPubKey, this.network);
        ...
        public virtual async Task<WalletBuildTransactionModel> OfflineSignRequest(OfflineSignRequest request, CancellationToken cancellationToken)
        {
            return await Task.Run(() =>
            {
                ...
                foreach (UtxoDescriptor utxo in request.Utxos)
                {
                    ...
                    coins.Add(new Coin(uint256.Parse(utxo.TransactionId), uint.Parse(utxo.Index), Money.Parse(utxo.Amount), scriptPubKey, Emoji.GetRandomPureBreed()));
