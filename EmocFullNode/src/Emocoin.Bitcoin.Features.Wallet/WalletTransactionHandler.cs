...
namespace Emocoin.Bitcoin.Features.Wallet
{
    ...
    public class WalletTransactionHandler : IWalletTransactionHandler
    {
        ...
        protected virtual void InitializeTransactionBuilder(TransactionBuildContext context)
        {
            ...
            //determin emoji slots in this.AddCoins
            this.AddCoins(context);
            //don't chage function call ordering , 
            //this.AddCoins determin new emoji slots and keep it in contex
        ...
        private Emoji determin_recept_emoji_slots(List<Coin> coins, long totalToSend)
        {
            Dictionary<uint, double> emojiBals = new Dictionary<uint, double>();
            long sum = 0;

            //split based on the emoji
            foreach(Coin coin in coins)
            {
                var genes = coin.TxOut.Emoji.Genes();
                long used = Math.Min(totalToSend - sum, coin.Amount);                
                double bal = used / genes.Length;
                sum += used;
                ...
                foreach (var gene in genes)
                {
                    uint key = gene.Code;
                    double preVal = 0;
                    if (emojiBals.TryGetValue(key, out preVal))
                    {
                        preVal += bal;
                        emojiBals[key] = preVal;
                    }
                    else
                    {
                        preVal = bal;
                        emojiBals.Add(key, preVal);
                    }
                }
                if (sum >= totalToSend) 
                    break;
            ...
            if (emojiBals.Count == 1)
            {
                var code = emojiBals.Keys.First();
                return new Emoji(code, code, code, code);
            }

            //sort descending by bal
            var balList = emojiBals.Select(bal => new Tuple<uint, double>(bal.Key, bal.Value)).ToList();
            balList.Sort(delegate (Tuple<uint, double> x, Tuple<uint, double> y)
            {
                return y.Item2.CompareTo(x.Item2);
            });

            //generate emoji codes
            var codeList = new List<uint>();
            foreach(var bal in balList)
            {
                int percent = (int)Math.Truncate(bal.Item2 * 100 / totalToSend);
                if(percent >= 85)
                {
                    codeList.Add(bal.Item1); codeList.Add(bal.Item1); 
                    codeList.Add(bal.Item1); codeList.Add(bal.Item1);
                }else if(percent >= 65)
                {
                    codeList.Add(bal.Item1); codeList.Add(bal.Item1); codeList.Add(bal.Item1);
                }
                else if (percent >= 35)
                {
                    codeList.Add(bal.Item1); codeList.Add(bal.Item1); 
                    ...
                    codeList.Add(bal.Item1);
            }

            if (codeList.Count==3)
            {
                codeList.Insert(0, codeList.First());
            }

            if (codeList.Count < 4)
            {                
                throw new WalletException("new emoji slots can't find");
            }
            return new Emoji(codeList[0], codeList[1], codeList[2], codeList[3]);
        }
        ...
        {            
            ...
            if (context.SelectedInputs != null && context.SelectedInputs.Any())
            {
                ...
                foreach (OutPoint outPoint in context.SelectedInputs)
                {
                    ...
                    coins.Add(new Coin(item.Transaction.Id, (uint)item.Transaction.Index, item.Transaction.Amount, item.Transaction.ScriptPubKey, item.Transaction.Emoji));
                    sum += item.Transaction.Amount;
                }
            }

            for(int iInput=0; iInput< context.Emojis.Count; iInput++)
            {
                var emoji = context.Emojis[iInput];
                foreach (UnspentOutputReference item in context.UnspentOutputs
                    .Where(a => a.Transaction.Emoji == emoji)
            ...
            //foreach (UnspentOutputReference item in context.UnspentOutputs
            //    .Where(a => a.Transaction.Emoji.ToString() == context.Emoji)
            //    .OrderByDescending(a => a.Confirmations > 0)                
            //    .ThenByDescending(a => a.Transaction.Amount))
            //{
            //    if (context.SelectedInputs?.Contains(item.ToOutPoint()) ?? false)
            ...
            var receptEmoji =  determin_recept_emoji_slots(coins, totalToSend);
            context.Emoji = receptEmoji.ToString();
        }
        ...
        protected virtual void AddRecipients(TransactionBuildContext context)
        {
            ...
            if (totalSubtractingRecipients == 0)
            {
                ...
                {
                    context.TransactionBuilder.Send(recipient.ScriptPubKey, recipient.Amount, context.Emoji);
                }
                return;
            }
