...
namespace Emocoin.Bitcoin.Configuration.Logging
{
    /// <summary>
    ...
    public static class LoggingConfiguration
    {
        ...
        private static readonly Dictionary<string, string> KeyCategories = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // { "lock", "" },
            // { "mempoolrej", "" },
            { "net", $"{nameof(Emocoin)}.{nameof(Bitcoin)}.{nameof(Connection)}.*" },
            // { "proxy", "" },
            // { "prune", "" },
            // { "rand", "" },
            // { "reindex", "" },
            ...
            // Short Names
            { "configuration", $"{nameof(Emocoin)}.{nameof(Bitcoin)}.{nameof(Configuration)}.*" },
            { "fullnode", $"{nameof(Emocoin)}.{nameof(Bitcoin)}.{nameof(FullNode)}" }
        };
        ...
        private static void AddFilters(LogSettings settings = null, DataFolder dataFolder = null)
        {
            ...
            var defaultRule = new LoggingRule($"{nameof(Emocoin)}.*", settings.LogLevel, mainTarget);
            ...
            if (settings.DebugArgs.Any() && settings.DebugArgs[0] != "1")
            {
                lock (KeyCategories)
                {
                    ...
                    foreach (string key in settings.DebugArgs)
                    {
                        ...
                        {
                            // Allow direct specification - e.g. "-debug=Emocoin.Bitcoin.Miner".
                            category = key.Trim();
                        }
            ...
            if (settings != null)
            {
                if (settings.DebugArgs.Any())
                {
                    if (settings.DebugArgs[0] == "1")
                    {
                        ...
                        builder.AddFilter($"{nameof(Emocoin)}.{nameof(Bitcoin)}", Microsoft.Extensions.Logging.LogLevel.Debug);
                    }
                    else
                    {
                        lock (KeyCategories)
