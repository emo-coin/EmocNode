...
namespace Emocoin.Bitcoin.Configuration
{
    ...
    public class NodeSettings : IDisposable
    {
        ...
        /// This includes separate subfolders for different nodes that run on the machine: a Emocoin folder for a
        /// Emocoin node, a Bitcoin folder for a Bitcoin node, and folders for any sidechain nodes. This value is
        ...
        public NodeSettings(Network network = null, ProtocolVersion protocolVersion = SupportedProtocolVersion,
            string agent = "EmocoinFullNode", string[] args = null, NetworksSelector networksSelector = null)
        {
            ...
            //string defDataDir = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(path), "data");
            string defDataDir = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
            defDataDir = Path.Combine(defDataDir, "EMOCWallet");
            string defConfName = "emocoin.conf";
            ...
            this.ConfigurationFile = this.ConfigReader.GetOrDefault<string>("conf", defConfName, this.Logger)?.NormalizeDirectorySeparator();
            this.DataDir = this.ConfigReader.GetOrDefault<string>("datadir", defDataDir, this.Logger)?.NormalizeDirectorySeparator();
            //seems defaut to the network name eg: .config/EMOCWallet/emoc/EmocRegTest
            this.DataDirRoot = this.ConfigReader.GetOrDefault<string>("datadirroot", "EmocNode", this.Logger);
            ...
            if(this.DataDir != null && !Directory.Exists(this.DataDir) &&
                Directory.CreateDirectory(this.DataDir) == null)
                throw new ConfigurationException($"Configuration Data directoy can not be created at {this.DataDir}.");
            ...
            if (this.ConfigurationFile != null)
            {
                ...
                if (!File.Exists(this.ConfigurationFile))
                {
                    try
                    {
                        File.WriteAllBytes(this.ConfigurationFile, global::Emocoin.Bitcoin.Properties.Resources.emocoin);
                    }catch(Exception ee)
                    {
                        throw new ConfigurationException($"Configuration file does not exist at {this.ConfigurationFile}.");
                    }                    
        ...
        public static void PrintHelp(Network network)
        {
            ...
            builder.AppendLine($"-datadirroot=<Path>       The path to the root data directory, which holds all node data on the machine. Defaults to 'EmocoinNode'.");
            builder.AppendLine($"-debug[=<string>]         Set 'Debug' logging level. Specify what to log via e.g. '-debug=Emocoin.Bitcoin.Miner,Emocoin.Bitcoin.Wallet'.");
