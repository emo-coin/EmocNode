...
namespace Emocoin.Features.SQLiteWalletRepository.External
{
    ...
    public abstract class TransactionsToListsBase : ITransactionsToLists
    {
        ...
        public abstract void RecordSpend(HashHeightPair block, TxIn txIn, string emoji, string pubKeyScript, bool isCoinBase, long spendTime, Money totalOut, uint256 spendTxId, int spendIndex);
        ...
        public bool ProcessTransactions(IEnumerable<Transaction> transactions, HashHeightPair block, uint256 fixedTxId = null, long? blockTime = null)
        {
            ...
            {                
                ...
                //find emoji to record spendable trx

                string Emoji = "";
                //Money maxOut = Money.Zero;

                for (int ii = 0; ii < tx.Outputs.Count; ii++)
                {
                    TxOut txOut = tx.Outputs[ii];
                    if (txOut.IsEmpty || txOut.Value == Money.Zero ||
                        txOut.Emoji == null || txOut.Emoji.isEmpty())
                        continue;

                    //if(txOut.Value != tx.TotalOut)
                    //    continue;
                    //if(txOut.Value > maxOut)
                    //{
                    //    Emoji = txOut.Emoji.ToString();
                    //    maxOut = txOut.Value;
                    //}
                    ...
                    bool isChange = destinations.Any(d => 
                        addressesOfInterest.Contains(d.ScriptPubKey, out AddressIdentifier address2) && 
                ...
                for (int i = 0; i < tx.Inputs.Count; i++)
                {
                    ...
                    if (transactionsOfInterest.Contains(txIn.PrevOut, out HashSet<AddressIdentifier> addresses))
                    {
                        ...
                        foreach (AddressIdentifier address in addresses)
                        {
                            ...
                            //{
                            //    TxOut txOut = tx.Outputs[ii];
                            //    if (txOut.ScriptPubKey.ToHex() != address.ScriptPubKey)
                            //        continue;
                            //}
                            RecordSpend(block, txIn, Emoji, address.ScriptPubKey, tx.IsCoinBase | tx.IsCoinStake, blockTime ?? currentTime, tx.TotalOut, txId, i);
                        }                       
        ...
        internal void ProcessTransactionData(HdAddress address, ICollection<TransactionData> transactions)
        {
            ...
            foreach (TransactionData transactionData in transactions)
            {
                ...
                if (spendingDetails != null)
                {
                    ...
                    string Emoji = "";
                    if (transactionData.Emoji != null)
                        Emoji = transactionData.Emoji.ToString();

                    this.RecordSpend(block, new TxIn() { PrevOut = new OutPoint(transactionData.Id, transactionData.Index) }, 
                        Emoji,
